			...
			cd(heating_data_dir*"/"*Node);
				...
				if Plant == "D"
					...
					for j = 1:3
						...
						if j == 3
							...
							D_data_heating = insert!(D_data_heating, j+1, ambient_column.u, Symbol.("Ambient_1_$sensor_number"));
	
				global D_data_heating = insert!(D_data_heating, j+2, date_column.date, Symbol.("Date"));
				...
				global C_data_heating = insert!(C_data_heating, j, column.w, Symbol.("TempPlant$sensor_number"));
						...
						if j == 3
							ambient_column = DataFrame(u = file[:,:14]);
							date_column = DataFrame(date = file[:,:1]);
						    C_data_heating = insert!(C_data_heating, j+1, ambient_column.u, Symbol.("Ambient_1_$sensor_number"));

							global C_data_heating = insert!(C_data_heating, j+2, date_column.date, Symbol.("Date"));
				...
				if Plant == "D"
					...
					for j = 1:3
						...
						if j == 3
							ambient_column = DataFrame(u = file[:,:14]);
							date_column = DataFrame(date = file[:,:1]);
						 	D_data_otc = insert!(D_data_otc, j+1, ambient_column.u, Symbol.("Ambient_1_$sensor_number"));

							global D_data_otc = insert!(D_data_otc, j+2, date_column.date, Symbol.("Date"));
				...
				elseif Plant == "C"
					...
					for j = 1:3
						...
						if j == 3
							ambient_column = DataFrame(u = file[:,:14]);
							date_column = DataFrame(date = file[:,:1]);
							C_data_otc = insert!(C_data_otc, j+1, ambient_column.u, Symbol.("Ambient_1_$sensor_number"));

							global C_data_otc = insert!(C_data_otc, j+2, date_column.date, Symbol.("Date"));
				...
				if Plant == "D"
					...
					for j = 1:3
						...
						if j == 3
							ambient_column = DataFrame(u = file[:,:14]);
							date_column = DataFrame(date = file[:,:1]);
							D_data_openspace = insert!(D_data_openspace, j+1, ambient_column.u, Symbol.("Ambient_1_$sensor_number"));

							global D_data_openspace = insert!(D_data_openspace, j+2, date_column.date, Symbol.("Date"));
						...
						if Node == 21 
							...
							SeparateByDate(D_data_openspace);
						end
				...
				elseif Plant == "C"
					...
					for j = 1:3
						...
						if j == 3
							ambient_column = DataFrame(u = file[:,:14]);
							date_column = DataFrame(date = file[:,:1]);
							C_data_openspace = insert!(C_data_openspace, j+1, ambient_column.u, Symbol.("Ambient_1_$sensor_number"));

							global C_data_openspace = insert!(C_data_openspace, j+2, date_column.date, Symbol.("Date"));
							...
							if Node == 21 
								...
								println("now you have new dataframe call 'D_data_openspace_21' enjoy! "); 
								SeparateByDate(C_data_openspace); 
							end
...
common_dir = pwd();
		...
		if Node in NodesOpenSpace
			 ...
			 Date = SubString(Data_frame.Date[1], 1, 10);
			
				for x = 1:size_df[1]
      		
					if SubString(Data_frame.Date[x], 1, 10) != Date || x == size_df[1]

						date = SubString(Data_frame.Date[x-1], 1, 10);
						new_dateframe = Data_frame[j+1:x-1,:];
						global j = size(new_dateframe)[1] + j; 
							...
							if Plant == "D"
								...
								CSV.write("$date Only_D.csv", new_dateframe);
							end	
							if Plant == "C"
								...
								CSV.write("$date Only_C.csv", new_dateframe);
							end	
						
										
					end #! Date 
					
					 Date = SubString(Data_frame.Date[x], 1, 10);		
